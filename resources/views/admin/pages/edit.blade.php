            ...
            <form action="{{ route('admin.pages.update', $page) }}" method="POST">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Changing the slug will change the page URL</p>
                        @error('slug')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="content" id="content" rows="20" required
                                  class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent dark:bg-gray-700 dark:text-gray-200">{{ old('content', $page->content) }}</textarea>
                        @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="border-t border-gray-200 dark:border-gray-700 pt-6 mt-6">
                        ...
                        <div>
                            ...
                            <textarea name="meta_description" id="meta_description" rows="3"
                                      class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent dark:bg-gray-700 dark:text-gray-200"
                                      placeholder="Optional: Brief description for search engines">{{ old('meta_description', $page->meta_description) }}</textarea>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <span class="text-gray-900 dark:text-gray-100 ml-2">{{ $page->created_at->format('M d, Y H:i') }}</span>
                            </div>
                            <div>
                                <span class="text-gray-500 dark:text-gray-400">Last Updated:</span>
                                <span class="text-gray-900 dark:text-gray-100 ml-2">{{ $page->updated_at->format('M d, Y H:i') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
